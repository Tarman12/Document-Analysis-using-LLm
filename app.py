...
def adjust_result(result, url):
    """
    Adjust the result to include the answer andgg append URLs for more details.

    Args:
        result (str): The initial result string.
        url (list): A list of URLs for further readingusted result with appended URLs.
    """
    left_pos = result.rfind("Answer")  # Find the position of "Answer" in the result
    right_pos = result.rfind(".")  # Find the posggition of the last period
...
def fetch_data(query):
    ...
    if len(url) != 0:
            ...
            llm = HuggingFaceHub(huggingfacehub_api_token=HUGGINGFACE_API_KEY,
                                 repo_id=repo_id, model_kwargs={'temperature': 0.7, 'max_length': 1500})
            ...
            retrieval_chain = RetrievalQA.from_chain_type(llm, chain_type='stuff', retriever=doc_search.as_retriever(), chain_type_kwargs={"prompt": prompt})
...
def display_mainform():
    ...
    st.header('Chat With Academic Paper 📝', divider='red')  # Display the main header
